...
class Module():
  def __init__(self, images_annotations, syllable, name, segment_definitions, user_stats = None, module_stats = None, n_syllables = 3, use_syntax_scores = True, print=True):
    self.module_name = name
    self.user_stats = user_stats
    self.n_syllables = n_syllables
    self.module_stats = module_stats
    self.syllables = [syllable(images_annotations, segment_definitions, self.get_user_stats_dict(user_stats), self.get_module_stats_dict(module_stats), print=print, use_syntax_scores=use_syntax_scores) for i in range(self.n_syllables)]
